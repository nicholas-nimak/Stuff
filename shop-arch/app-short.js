...
function showGoods() {
  ...
  $('main').append(`<section id='${this.section_id}'></section>`);

  this.goods.forEach(good => {
    new Promise(resolve => {
      $(`section#${this.section_id}`).append(`
          <form name='${good.id}' onsumbit='return false'>
              <h3>${good.title}</h3>
              <p>коштують <b>${good.price}</b> гривень</p>
              <p>в наявності ${good.unit}: <b>${good.quantity}</b></p>
              <input type="number" name="add_to_cart_input" min="1" max="${good.quantity}" value="1"></input>
              ${good.unit}
              <button name='add_to_cart_good_${good.id}' onclick='return false'>ДОДАТИ В КОШИК</button>
          </form>
        `);
        resolve();
    }).then(
      () => {
        $(`button[name=add_to_cart_good_${good.id}]`).click(() => {console.log('hello')})
      }
    )
  })
}
